...
void main() {
   printf("ACO350: Advisement - Bryan Bermejo\n\n");
...
void *professor(void *param) {	
   ...
   while (TRUE) {
	...
	//printf("Professor: In the office\n");
	sem_wait(&studentReady); //wait for student to be ready once we get one we safely increase #offreeseats

	sem_wait(&accessHallSeats);//get lock if no one has it. 
		numberFreeHallSeats++; // safely increase number 
	printf("Number of seats available are: %d\n", numberFreeHallSeats);
	sem_post(&accessHallSeats);//release Lock
...
void *student(void *param) {
   ...
   while (TRUE) {
	...
	if (numberFreeHallSeats > 0) {
		sem_wait(&accessHallSeats);//just testing this location out
		numberFreeHallSeats--;
		printf("Number of seats available: %d\n", numberFreeHallSeats);
		sem_post(&accessHallSeats);

		sem_post(&studentReady);
		sem_wait(&professorReady);//we wait for prof to be ready but idk if we use semwait(profRready)
